function calcSLA() {
  ...
  const outros     = total - entregues - emRota - ocorr - faltante;
  const faltamMeta = total > 0 ? Math.max(0, Math.ceil(0.98 * total) - entregues) : 0;
  return { total, entregues, emRota, ocorr, faltante, outros, faltamMeta, pct: total > 0 ? (entregues / total * 100) : 0 };
}
